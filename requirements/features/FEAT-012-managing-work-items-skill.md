...
└── managing-work-items/
    ...
    └── references/
        ├── github-templates.md     # Consolidated from executing skills
        └── jira-templates.md       # New Jira comment templates
```
...
managing-work-items close <issue-ref> --context <json-context>
```
...
| Close | `gh issue close <N> --comment "<formatted-comment>"` |
...
|-----------|----------|------|
| Fetch | Rovo MCP `getIssue` tool | `acli jira getIssue --issue PROJ-123` |
| Comment | Rovo MCP `addComment` tool | `acli jira addComment --issue PROJ-123 --comment "..."` |
| Close/Transition | Rovo MCP `transitionIssue` tool | `acli jira transitionIssue --issue PROJ-123 --transition "Done"` |
...
Each comment type has both a GitHub template and a Jira template in the references directory.

### FR-6: Close/Transition Operation
Close (GitHub) or transition (Jira) the issue with a summary comment:

**GitHub**: `gh issue close <N> --comment "<summary>"`
**Jira**: Transition to "Done" (or equivalent) with comment

Used by:
- Orchestrator at finalization for all chains (feature, chore, and bug) via `managing-work-items close`

### FR-7: PR Body Issue Link Generation
...
### FR-8: Issue Reference Extraction from Documents
...
- Close/transition operations should handle already-closed/transitioned issues gracefully
...
- Templates should produce equivalent information across GitHub and Jira formats

## Refactoring Requirements

### Skills to Modify

| Skill | Operations to Remove | Replacement |
|-------|---------------------|-------------|
| `implementing-plan-phases` | `gh issue comment` (phase start/completion), `gh issue close` (final phase) | Orchestrator invokes `managing-work-items` before/after each phase |
| `executing-chores` | `gh issue comment` (start/completion), `Closes #N` in PR body | Orchestrator invokes `managing-work-items`; PR body link via FR-7 |
| `executing-bug-fixes` | `gh issue comment` (start/completion), `Closes #N` in PR body | Orchestrator invokes `managing-work-items`; PR body link via FR-7 |
| `documenting-features` | `gh api` issue fetch for `#N` pre-fill | Orchestrator invokes `managing-work-items fetch` before step 1 (or skill delegates to orchestrator) |
| `documenting-chores` | Issue link storage pattern | No change to storage — extraction centralized in FR-8 |
| `documenting-bugs` | Issue link storage pattern | No change to storage — extraction centralized in FR-8 |
...
| At PR creation | Generate issue link via FR-7 | Include in PR body |
| At finalization | `close` | Close/transition the issue |
...
2. **Issue already closed**: Close operation encounters already-closed issue — handle without error
3. **Multiple issue references**: Document contains both `#N` and `PROJ-123` — use the first reference found
4. **Invalid issue reference**: `#N` where N doesn't exist — log warning from `gh issue view` failure, skip
5. **Jira project key with numbers**: `PROJ2-123` — regex must handle alphanumeric project keys
6. **Network unavailable mid-workflow**: Issue operation fails after earlier operations succeeded — skip and continue; comments may be partially posted
7. **Permission denied**: User lacks permission to comment/close — log error, skip operation
8. **Long comment bodies**: Template expansion produces very large comments — no truncation (GitHub/Jira handle their own limits)
...
### Integration Tests
- GitHub fetch/comment/close operations via `gh` CLI
- Graceful degradation when `gh` is not available
- Jira fallback chain (Rovo MCP → acli → skip)
- Idempotent close on already-closed issues
